...
def regrid(adapter: RegriddingServiceAdapter, input_filepath: str,
    ...
    with Dataset(input_filepath, mode='r') as source_ds, \
        ...
        crs_map = _write_grid_mappings(target_ds,
                                       _resampled_dimension_pairs(var_info),
                                       target_area)

        vars_to_process = var_info.get_all_variables()
        cloned_vars = _clone_variables(source_ds, target_ds,
                                       _unresampled_variables(var_info))
        adapter.logger.info(f'cloned variables: {cloned_vars}')
        vars_to_process -= cloned_vars

        dimension_vars = _copy_dimension_variables(source_ds, target_ds,
                                                   target_area, var_info)
        adapter.logger.info(f'processed dimension variables: {dimension_vars}')
        vars_to_process -= dimension_vars

        resampled_vars = _resample_nD_variables(source_ds, target_ds, var_info,
                                                resampler_cache, set(vars_to_process))
...
def _add_grid_mapping_metadata(target_ds: Dataset, variables: Set[str],
                               var_info: VarInfoFromNetCDF4,
                               crs_map: Dict) -> None:
    """Link regridded variables to the correct crs variable."""
    for var_name in variables:
        crs_variable_name = crs_map[_horizontal_dims_for_variable(
            var_info, var_name)]
        var = _get_variable(target_ds, var_name)
        var.setncattr('grid_mapping', crs_variable_name)


def _resample_variable_data(s_var: np.ndarray, t_var: np.ndarray, resampler: DaskEWAResampler,
                            var_info: VarInfoFromNetCDF4, var_name: str) -> None:
    ...
    if len(s_var.shape) > 2:
        for layer_index in range(s_var.shape[0]):
            t_var[layer_index, ...] = _resample_variable_data(
                s_var[layer_index, ...], t_var[layer_index, ...],
                resampler, var_info, var_name
            )
        return t_var
    else:
...
def _resample_nD_variables(source_ds: Dataset, target_ds: Dataset,
                           var_info: VarInfoFromNetCDF4, resampler_cache: Dict,
    ...
    for var_name in variables:
        ...
        t_var[:] = _resample_variable_data(s_var[:], t_var[:], resampler, var_info, var_name)
    return variables


def _resample_layer(source_plane: np.ma.array, resampler: DaskEWAResampler,
                    var_info: VarInfoFromNetCDF4, var_name: str) -> np.ma.array:
    """Prepare the input layer, resample and return the results."""
    prepped_source = _prepare_data_plane(source_plane, var_info, var_name)
    target_data = resampler.compute(prepped_source,
                                    **_resampler_kwargs(prepped_source))
    return _prepare_data_plane(target_data, var_info,
                               var_name).astype(source_plane.dtype)
...
def _best_cast(integer_type: np.dtype) -> np.dtype:
    ...
    return next(float_type for float_type in float_types
                if np.can_cast(integer_type, float_type))


def _prepare_data_plane(data: np.Array, var_info: VarInfoFromNetCDF4,
                        var_name: str) -> np.Array:
...
def _needs_rotation(var_info: VarInfoFromNetCDF4, variable: str) -> bool:
    ...
    xloc = next((index for index, dimension in enumerate(var_dims)
                 if _is_projection_x_dim(dimension, var_info)), None)
    yloc = next((index for index, dimension in enumerate(var_dims)
                 if _is_projection_y_dim(dimension, var_info)), None)
    if yloc > xloc:
        needs_rotation = True
...
def _get_bounds_var(var_info: VarInfoFromNetCDF4, dim_name: str) -> str:
    return next((var_info.get_variable(f'{dim_name}_{ext}').name
...
def _transfer_dimensions(source_ds: Dataset, target_ds: Dataset,
                         target_area: AreaDefinition,
                         var_info: VarInfoFromNetCDF4) -> None:
    ...
    _create_resampled_dimensions(resampled_dimension_pairs, target_ds,
                                 target_area, var_info)


def _write_grid_mappings(target_ds: Dataset,
                         resampled_dim_pairs: List[Tuple[str, str]],
                         target_area: AreaDefinition) -> Dict:
...
def _crs_variable_name(dim_pair: Tuple[str, str],
                       resampled_dim_pairs: List[Tuple[str, str]]) -> str:
...
def _clone_variables(source_ds: Dataset, target_ds: Dataset,
    ...
    for dimension_name in dimensions:
        (s_var, t_var) = _copy_var_with_attrs(source_ds, target_ds,
                                              dimension_name)
        t_var[:] = s_var[:]
...
def _copy_var_with_attrs(source_ds: Dataset, target_ds: Dataset,
    ...
    """
    s_var, t_var = _copy_var_without_metadata(source_ds, target_ds,
                                              variable_name)
...
def _copy_var_without_metadata(source_ds: Dataset, target_ds: Dataset,
                               variable_name: str) -> (Variable, Variable):
...
def _create_resampled_dimensions(resampled_dim_pairs: List[Tuple[str, str]],
                                 dataset: Dataset, target_area: AreaDefinition,
                                 var_info: VarInfoFromNetCDF4):
    ...
    for dim_pair in resampled_dim_pairs:
        ...
        _create_dimension(dataset, xdim,
                          target_area.projection_x_coords.shape[0])
        _create_dimension(dataset, ydim,
                          target_area.projection_y_coords.shape[0])
...
def _horizontal_dims_for_variable(var_info: VarInfoFromNetCDF4,
                                  var_name: str) -> Tuple[str, str]:
    """Return the horizontal dimensions for desired variable."""
    return next((dims for dims, var_names in
                 var_info.group_variables_by_horizontal_dimensions().items()
                 if var_name in var_names), None)
...
def _resampled_dimension_pairs(
        var_info: VarInfoFromNetCDF4) -> List[Tuple[str, str]]:
...
def _get_projection_x_dims(dims: Iterable[str],
...
def _get_projection_y_dims(dims: Iterable[str],
